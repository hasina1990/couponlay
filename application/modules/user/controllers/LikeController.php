<?php
class User_LikeController extends Ccc_Controller_Action_Front
{
    ...
    public function indexAction()
    {
        try
        {
            ...
            $couponId = (int)$this->getRequest()->getParam("coupon", 0);
            ...
            if($couponId)
            {
                $select->where("coupon_id = ?", $couponId); 
            }                      
            elseif($storeId)
            {
            ...
            $voting = $votingModel->fetchRow($select); 
            if(!$voting)
            {
                $voting = $votingModel->createRow();
                $voting->ip_address = $ip;
                $voting->coupon_id = $couponId;
                $voting->website_id = $storeId;
                $voting->vote = (float)$this->getRequest()->getParam("value", 0);
                $voting->created_date = date("Y-m-d H:i:s");
                $voting->save();
            }
            elseif($couponId)
            {
                throw new Exception("You have already voted on this offer!");
            }
            ...
            if((int)$couponId)
            {
                ...
                $avarageValue =  count($votingModel->getAdapter()->fetchCol($select));
            }
            elseif((int)$storeId)
            {
                $votingModel = Ccc::getModel("voting/voting");
                ...
                $avarageValue = $currentVote;
                if($totalUsers > 0 || $currentVote > 0)
                {
                    $avarageValue = (float)$currentVote/$totalUsers;
                }
            }
           ...
           $response = array(
                "error" => 0,
                "message" => "Your Vote is saved Successfully.",
