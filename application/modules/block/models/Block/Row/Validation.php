...
class Block_Model_Block_Row_Validation extends Core_Model_Table_Row_Validation_Abstract
{
    ...
    {    
        ...
        {  
            $this->setError($key, 'Please enter valid block name.');
            return false;
        }
        return true;
    }
    ...
    {    
        ...
        if(!$valid->isValid(($row->{$key})) )
        {  
            $this->setError($key, 'Please enter valid block title.');
            return false;
        }
        return true;
    }
    ...
    {    
        ...
        if(!$valid->isValid(($row->{$key})) )
        {  
            $this->setError($key, 'Please enter valid block content.');
            return false;
        }
        return true;
    }
    ...
    {
        if(!$this->getRowClassObject()->block_id)
        {
            if(!$this->_getDateModel()->isDate($this->getRowClassObject()->{$key}))
            {
                return false;
