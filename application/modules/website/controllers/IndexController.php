...
class Website_IndexController extends Ccc_Controller_Action_Front
{
    ...
    {
        $block = Ccc::getBlock('website/index_review');
        $this->getResponse()->appendBody($block->toHtml());
        $this->getHelper('viewRenderer')->setNoRender(true);
    }
    ...
    public function saveReviewAction()
    {
        ...
        {   
            ...
            if(!isset($review["captcha"]) || !(int)($review["captcha"]))
            {
                throw new Exception('Wrong answer of expression.');
            }            
            $captcha = Ccc::getModel('user/session')->getSession('captcha',array());
            
            if(!is_array($captcha) || !isset($captcha[0]) || !isset($captcha[1]))
            {
                throw new Exception('Please refresh the Page to add your review.');
            }
            if($review['captcha'] != ($captcha[0] + $captcha[1]))
            {
                throw new Exception('Wrong answer of expression.');
            }
            ...
            $reviewRow->save();
            $content = Ccc::getBlock("website/index_review")->setWebsite($website)->toHtml();
            $response = array(
