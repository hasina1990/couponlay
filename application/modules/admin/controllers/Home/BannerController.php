<?php
class Admin_Home_BannerController extends Ccc_Controller_Action_Admin
{
    public function indexAction()
    {
        try
        {
            $this->_setTitle('Manage Banners')->_setActiveTab('system');        
            $this->getResponse()->appendBody(Ccc::getBlock("admin/home_banner_index")->toHtml());
            ...
            $response = array(
                ...
                'content'=>array(
                    array(
                        ...
                        'html'=>Ccc::getBlock("admin/home_banner_index")->toHtml(),
                    )
                ),
                'message'=>''
            );
        ...
        { 
            $response = array(
                ...
                'content'=>array(
                    array(
                        'element'=>'grid-list',
                        'html'=>Ccc::getBlock("admin/home_banner_grid")->toHtml(),
                    )
                ),
                'message'=>''
            );
        ...
        { 
            $response = array(
                ...
                'content'=>array(
                    array(
                        'element'=>'main-container',
                        'html'=>Ccc::getBlock("admin/home_banner_create")->toHtml(),
                    )
                ),
                'message'=>''
            );
    ...
    public function saveAction()
    {
        try
        {
            ...
            $banner->is_ad = Banner_Model_Banner::IS_AD_NO;
            if(!$banner->validate())
            {
            ...
            if(count($_FILES))
            {
                Ccc::getModel('banner/uploader')->setBanner($banner)->setFile($_FILES['image'])->saveBannerImage();
            }
            
            $response = array(
                'responseType'=>"success",
                'message'=>$this->_getTranslate()->translate('Slider was saved successfully.'),
                'action'=>array(
                    array(
                        'element'=>'main-container',
                        'url'=>$this->view->baseUrl('admin/home_banner/index-json')
                    )),
            );
        }
    ...
    public function deleteAction()
    {
        try
        {
            ...
            $message  = $this->_getTranslate()->translate("%s Sliders(s) were deleted successfully.");
